                    ...
                    Button { showProfile = true } label: {
                        ZStack {
                            ...
                            Image(systemName: "person.fill")
                                ...
                                .foregroundStyle(Color(hex: "8B5E3C"))
                        }
                    }
                }
            }
...
// ═════════════════════════════════════════════════════════════════════
// MARK: - Milestone Gauge (Apple-style)
// ═════════════════════════════════════════════════════════════════════

struct MilestoneGauge: View {
    let currentPoints: Int
    let milestones: [Int]

    private var maxVal: Int { milestones.last ?? 15_000 }

    private var progress: CGFloat {
        min(CGFloat(currentPoints) / CGFloat(maxVal), 1.0)
    }
        ...
        GeometryReader { geo in
            ...
            ZStack(alignment: .topLeading) {
                ...
                ForEach(milestones, id: \.self) { ms in
                    let frac = CGFloat(ms) / CGFloat(maxVal)
                    let x = w * frac

                    // Dot on track
                    Circle()
                        .fill(currentPoints >= ms ? AppTheme.orange : Color(.systemGray4))
                        ...
                        .position(x: x, y: trackY + trackH / 2)
                ...
                Circle()
                    ...
                    .shadow(color: AppTheme.orange.opacity(0.3), radius: 3, x: 0, y: 1)
                    .position(x: max(6, w * progress), y: trackY + trackH / 2)
                    .animation(.easeOut(duration: 0.8), value: progress)
            }
        }
        .frame(height: 30)
    ...
    private func shortLabel(_ v: Int) -> String {
        "\(v / 1000),000"
    }
}
